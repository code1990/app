...
class Chapter0623ConstraintLayoutActivity : AppCompatActivity() {
    private var isMoved = false
    private var lastVieweId: Int = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_constraint_layout)
        lastVieweId = cl_content.id
    ...
    private fun moveView() {
        ...
        //设置该控件的左侧约束关系与间距
        set.connect(tv_first.id, ConstraintSet.START,
                cl_content.id, ConstraintSet.START, margin)
        //设置该控件的右侧约束关系与间距
